---
title: "Question 1 - Prediction using Machine Learning Approach with Physics-Informed Features"
...
This notebook provides a machine learning implementation for forecasting Germany's hourly solar power generation for June 2025. 
It expands the original data set with physics-based features that include:
...
The model architecture is implemented with proper regularization and uncertainty quantification,
...
try:
    ...
    print(f"✅ Solar observations: {solar_obs.shape}")
    print(f"✅ Meteorological data: {solar_obs.shape}")
...
def create_accurate_solar_features(data, latitude=51.1657, longitude=10.4515):
    ...
    """
    print("🌞 Creating accurate solar position features...")
    ...
    print(f"   ✅ Added {8} solar position features")
    return data
...
axes[0, 1].set_title('Solar Azimuth ')
...
## Physics-Based Clear Sky Modeling
...
def create_clear_sky_features(data, latitude=51.1657, longitude=10.4515):
    ...
    """
    print("☀️ Creating clear sky irradiance features...")
    ...
    try:
        ...
        print("   ✅ Using PVLib Ineichen clear sky model")
        
    except Exception as e:
        print(f"   ⚠️ PVLib error: {e}")
        print("   🔄 Using fallback clear sky model...")
    ...
    print(f"   ✅ Added clear sky irradiance components")
    return data
...
def create_enhanced_weather_features(data):
    ...
    """
    print("🌤️ Creating enhanced weather features...")
    ...
    print(f"   ✅ Added {12} enhanced weather features")
    return data
...
def create_comprehensive_temporal_features(data, target_col='power'):
    ...
    print(f"   ✅ Added {total_temporal_features} temporal features")
    return data
...
print("✅ Enhanced neural network architecture defined")
```
...
def robust_preprocessing(data, target_col='power'):
    ...
    if target_col in data_clean.columns:
        ...
        print(f"   📊 Detected {outlier_count} outliers ({100*outlier_count/len(data_clean):.2f}%)")
    ...
    print(f"   ✅ Preprocessing completed. Shape: {data_clean.shape}")
    return data_clean
...
print(f"✅ Data prepared for modeling: {X_train.shape[1]} features")
...
print("📊 Training Persistence Model (Baseline)...")

class PersistenceModel:
    """
    Persistence model with exponential weighting
    """
    def __init__(self):
    ...
    def fit(self, data, target_col='power'):
        ...
        print("   📈 Learning historical patterns...")
        ...
        print(f"   ✅ Learned patterns for {len(self.historical_patterns)} month-hour combinations")
...
print("🧠 Training Enhanced Feedforward Neural Network...")
...
print(f"📊 Best RMSE: {results_df.loc[best_model_name, 'RMSE']:.4f} MWh")
...
print("\n" + "="*80)
print("🎯 COMPREHENSIVE MODEL COMPARISON RESULTS (INCLUDING ALL MODELS)")
print("="*80)
display(results_df.round(4))
...
def generate_final_forecast():
    ...
    - Handle missing lag features in forecast period
    """
    print("🎯 Generating Final Forecast for June 2025...")
    ...
    print(f"   📅 Forecast period: {june_weather.index.min()} to {june_weather.index.max()}")
    print(f"   📊 Forecast hours: {len(june_weather)}")
    
    #except FileNotFoundError:
    #    print("   ⚠️ Using simulated weather data for demonstration")
    ...
    # Filter out lag features that won't exist in forecast period
    non_lag_features = [col for col in selected_features 
                       if not any(keyword in col.lower() for keyword in 
                                 ['lag', 'rolling', 'ewm', 'seasonal', 'trend'])]
    
    print(f"   🔧 Filtering out {len(selected_features) - len(non_lag_features)} lag/temporal features")
    ...
    print(f"   🔢 Using {len(forecast_features)} features for prediction")
    print(f"   📋 Available features: {forecast_features[:10]}..." if len(forecast_features) > 10 else f"   📋 Features: {forecast_features}")
    ...
    print("   🔄 Retraining models on full historical dataset...")
    ...
    print("   🔧 Applying physics-based post-processing...")
    ...
    print(f"   📊 Average predicted power: {submission_df['power'].mean():.2f} MWh")
    print(f"   📊 Peak predicted power: {submission_df['power'].max():.2f} MWh")
    print(f"   📊 Total daily energy (avg): {submission_df['power'].sum() / 30:.2f} MWh/day")
    print(f"   📊 Average uncertainty: {detailed_df['power_std'].mean():.2f} MWh")
    print(f"   🌙 Nighttime hours (zero power): {sum(is_night)} / {len(is_night)}")
...
print("🎯 FINAL FORECAST COMPLETED with uncertainty quantification!")
```
...
def analyze_feature_importance():
    ...
    # Get feature importance from tree-based models (use correct keys)
    if 'LightGBM' in models:
        lgb_importance = models['LightGBM'].feature_importances_
    elif 'Enhanced_LightGBM' in models:
    ...
    elif 'Enhanced_XGBoost' in models:
    ...
    if len(lgb_importance) != len(selected_features):
        print(f"   ⚠️ Feature mismatch: {len(lgb_importance)} importances vs {len(selected_features)} features")
        # Get the actual feature names from the model (if available)
        if hasattr(models['LightGBM'], 'feature_name_'):
            actual_features = models['LightGBM'].feature_name_
        else:
            # Use available features from training
            actual_features = [f"feature_{i}" for i in range(len(lgb_importance))]
    else:
        actual_features = selected_features
    
    # Create importance DataFrame
    importance_df = pd.DataFrame({
        'feature': actual_features[:len(lgb_importance)],  # Ensure same length
...
try:
    ...
    if feature_importance is not None:
        print("🔧 NEW FEATURE: Comprehensive feature importance analysis")
    else:
        print("⚠️ Feature importance analysis skipped due to missing models")
except Exception as e:
...
print(f"   📊 Best RMSE: {results_df.loc[best_model_name, 'RMSE']:.4f} MWh") 
print(f"   📊 Best R²: {results_df.loc[best_model_name, 'R²']:.4f}")
print(f"   📊 Ensemble RMSE: {ensemble_metrics['RMSE']:.4f} MWh")
print(f"   📊 Ensemble R²: {ensemble_metrics['R²']:.4f}")

print(f"\n📁 OUTPUT FILES:")
print(f"   📄 forecast_q1.csv - Main submission file")
print(f"   📄 forecast_q1_detailed.csv - With uncertainty bands")

print("="*80)
```
---

Based on the current results, we will focus on LightGBM  to deliver
the forecast. The results are presented in `answer_question_1.qmd`.
