---
title: "Question 2 (Expanded): Detecting True Demand Growth in a Solar-Rich Energy System"
author: "Gemini"
format: html
jupyter: python3
execute:
...
This is an **expanded and improved version** of the original analysis. It incorporates several methodological enhancements based on an expert review:

1.  **Accurate Holiday Modeling:** A proper German holiday calendar has been included to prevent anomalous demand on those days from skewing the results.
2.  **Unbiased Regression Model:** The regression-based BTM estimation model (Method 2) is now trained exclusively on non-solar hour data to remove a key source of bias and provide a more accurate baseline demand prediction.
3.  **Refined BTM Estimation:** The hardcoded linear growth assumption in the weather-based model (Method 1) has been removed in favor of a more data-driven approach.
...
5.  **What is the year-over-year growth in *true* demand?** (A new question to complete the analysis)
...
def add_temporal_features(df):
    ...
    # --- IMPROVEMENT: Add accurate German holiday calendar ---
...
## BTM Solar Estimation Methods (Revised)

### Method 1 (Revised): Demand Gap Analysis
...
class DemandGapBTMEstimator:
    ...
    def fit(self, data):
        ...
        # --- KEY IMPROVEMENT: Train only on non-solar hours ---
...
# Apply Revised Method 1
print("=== Method 1 (Revised): Demand Gap Analysis ===")
...
### Method 2 (Revised): Unbiased Regression Decomposition

This method uses a simple Ridge regression model but is now correctly trained **only on non-solar hour data** to prevent the model from learning the BTM-suppressed demand as "normal". This removes the primary source of bias from the original approach.

```{python}
#| label: method2-revised
...
# Apply Revised Method 2
print("=== Method 2 (Revised): Unbiased Regression Decomposition ===")
...
### Method 3: Time Series Decomposition (Unchanged)

This method remains a useful, independent approach that looks at the diverging trends between solar and non-solar hour demand over time. It is kept as-is from the original notebook to provide a third, complementary perspective.

```{python}
#| label: method3-unchanged
def estimate_btm_time_series(data):
    """
    Use time series decomposition to identify BTM solar trends.
    This method is conceptually different and serves as a good third estimate.
    """
    results = data.copy()
...
## Comprehensive BTM Solar Analysis (Revised)

### Combine Methods and Calculate True Demand

```{python}
#| label: combine-methods-revised
...
# Calculate ensemble estimate (equal weighting for the two primary, improved methods)
...
print("=== BTM Solar Estimation Comparison (Revised Models) ===")
...
### Final Question: What is the year-over-year growth in *true* demand?
