...
class PersistenceModel:
    ...
    def fit(self, data):
        ...
        weights = np.exp(time_delta / 365) # Exponentially decay over a year
...
The bar chart below clearly shows that **LightGBM** and **XGBoost** are the top-performing models, with LightGBM having a slight edge. The complex neural network models did not outperform the gradient boosting methods in this case.

```{python}
#| label: plot-comparison
#| fig-cap: "Comparison of Model Performance on the 2025 Validation Set"
plt.figure(figsize=(10, 6))
sns.barplot(x='RMSE', y='Model', data=results_df, palette='viridis', orient='h')
...
The scatter plot for the best model, LightGBM, shows a strong correlation between predicted and actual values. As noted, there is a slight tendency to under-predict the highest power peaks, a common characteristic of tree-based models. However, the overall performance is excellent, with an R² of **`r round(results_df.iloc[0]['R²'], 4)`**.

```{python}
#| label: plot-scatter-best
#| fig-cap: "Predicted vs. Actual Power for the Best Model (LightGBM)"
plt.figure(figsize=(8, 8))
sns.scatterplot(x=y_val, y=predictions[best_model_name], s=5, alpha=0.5)
plt.plot([0, y_val.max()], [0, y_val.max()], 'r--', lw=2, label='Perfect Forecast')
...
Having identified LightGBM as the best model, we use it to generate the final forecast.

1.  **Retrain the Model:** The LightGBM model is retrained on the **entire historical dataset** (2022-01-01 to 2025-05-31) to learn from all available information.
...
final_forecast_power = final_model.predict(forecast_data[features])
...
The plot below shows the final hourly forecast for the entire month of June 2025, as generated by the LightGBM model.
